...
void
gpu_gradient_minAdam_kernel(float* pMem_conformations_next,
...
//subjected to local search, the entity with ID num_of_lsentities is selected instead of the first one (with ID 0).
{
...
#endif
    float beta1p = 1.0f - cl::sycl::pow(cData.dockpars.adam_beta1,
                                    1.0f + iteration_cnt);
    float beta2p = 1.0f - cl::sycl::pow(cData.dockpars.adam_beta2,
                                    1.0f + iteration_cnt);
    ...
    for (int i = item_ct1.get_local_id(2);
        ...
        // Applying update
        genotype[i] -=
            mp / (cl::sycl::sqrt(vp) + cData.dockpars.adam_epsilon);
    }
    ...
    __threadfence();
    item_ct1
        .barrier(); // making sure that iteration_cnt is up-to-date
#ifdef AD_RHO_CRITERION
...
void gpu_gradient_minAdam(uint32_t blocks,
                          uint32_t threads,
                          ...
                          float* pMem_energies_next)
{
    ...
    get_sycl_queue().submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(cl::sycl::nd_range<3>(cl::sycl::range<3>(1, 1, blocks) * cl::sycl::range<3>(1, 1, threads),
                             ...
                             [=](cl::sycl::nd_item<3> item_ct1) {
                                 gpu_gradient_minAdam_kernel(pMem_conformations_next, pMem_energies_next,
#ifdef AD_RHO_CRITERION
                                                             rho_acc.get_pointer(),
                                                             ...
                                                             *cData_ptr, entity_id_acc.get_pointer(),
